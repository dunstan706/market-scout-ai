...
export function GlobeDashboard() {
  ...
  // Plans / upgrade overlay — also reachable from the nav pill's Plans item.
  ...
  return (
    <main className="theme-dark relative h-screen w-full overflow-hidden">
      ...
      {/* Add-business pill — expands into the add-a-business panel. The icon
          ...
          the overlay so it doubles as the close button. It fades out while a
          run-a-scan box is open so it never floats over the panel; the box has
          its own close (X / outside press / Escape), and the pill returns the
          moment the globe is back. */}
      <div
        className={cn(
          "fixed bottom-6 left-1/2 z-[60] -translate-x-1/2 transition-opacity duration-300",
          scanOpen && !adding && !pricingOpen ? "pointer-events-none opacity-0" : "opacity-100",
        )}
      >
        <button
          type="button"
          aria-label={
            adding ? "Close add a business" : pricingOpen ? "Close pricing" : "Add a business"
          }
          aria-expanded={adding || pricingOpen}
          onClick={() => (adding ? closeAdd() : pricingOpen ? closePricing() : openAdd())}
        ...
        >
          <Plus
            className={`h-5 w-5 transition-transform duration-300 ${adding || pricingOpen ? "rotate-45" : ""}`}
            aria-hidden="true"
          />
        </button>
      </div>
